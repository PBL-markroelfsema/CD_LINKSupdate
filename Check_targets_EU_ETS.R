...
write.table(Check_targets_NoPolicy_EU_ETS , "data/IndicatorOutput_NoPolicy_ETS.csv", sep=";", row.names=FALSE)
...
for (i in 1:nrow(Read_CD_LINKS_EU_ETS))
  #for (i in 20:20)
{ print(paste(Read_CD_LINKS_EU_ETS[i,2], "-",Read_CD_LINKS_EU_ETS[i,3]))
  if (as.character(Read_CD_LINKS_EU_ETS[i,3]) %in% names(EU_ETSi))
  {  tmp_EU_ETS<- filter(EU_ETSi[[as.character(Read_CD_LINKS_EU_ETS[i,3])]], region %in% c('WEU', 'CEU', 'EU'), year >= 1990, year <= 2050) %>% select(year, region, value, unit)
  }
  else # as.character(Read_CD_LINKS[i,3]) %in% names(NPi))
  { tmp_EU_ETS <- filter(EU_ETS[[as.character(Read_CD_LINKS_EU_ETS[i,3])]], region %in% c('WEU', 'CEU', 'EU'), year >= 1990, year <= 2050) %>% select(year, region, value, unit)
  }
...
write.table(Check_targets_EU_ETS, "data/IndicatorOutput_EU_ETS.csv", sep=";", row.names=FALSE)
