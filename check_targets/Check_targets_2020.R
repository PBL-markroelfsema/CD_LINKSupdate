...
Read_CD_LINKS_NoPolicy <- read.csv(file="data/IndicatorInput.csv", header=TRUE, sep=";") #or sep=";"
...
#for (i in 53:53)
#i=53
  { print(paste0(i, ": ", Read_CD_LINKS_NoPolicy[i,2], "-",Read_CD_LINKS_NoPolicy[i,3]))
  ...
  tmp_NoPolicy <- spread(tmp_NoPolicy, key=year, value=value) %>% as.data.frame()
  Check_targets_NoPolicy <- rbind(Check_targets_NoPolicy, tmp_NoPolicy) %>% as.data.frame()
}
...
{ print(paste(Read_CD_LINKS[i,2], "-",Read_CD_LINKS[i,3]))
  if (as.character(Read_CD_LINKS[i,3]) %in% names(NPii))
  {  tmp_NPi <- filter(NPii[[as.character(Read_CD_LINKS[i,3])]], region==as.character(Read_CD_LINKS[i,2]), year >= StartYear, year <= EndYear) %>% select(year, region, value, unit)
  }
  else # as.character(Read_CD_LINKS[i,3]) %in% names(NPi))
  { tmp_NPi <- filter(NPi[[as.character(Read_CD_LINKS[i,3])]], region==as.character(Read_CD_LINKS[i,2]), year >= StartYear, year <= EndYear) %>% select(year, region, value, unit)
  }
  tmp_NPi <- mutate(tmp_NPi, R_Variable = Read_CD_LINKS[i,3])
  tmp_NPi <- mutate(tmp_NPi, Policy_ID = Read_CD_LINKS[i,1])
...
write.table(Check_targets_NPi , "data/IndicatorOutput_NPi.csv", sep=";", row.names=FALSE)
