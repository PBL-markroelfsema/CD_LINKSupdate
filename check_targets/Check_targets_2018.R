...
NoPolicy_2018_i <- try(ProcessTimerScenario(NoPolicy_2018, Rundir, Project, Policy=TRUE))
...
{ 
  ...
  { try(tmp_NoPolicy_2018 <- filter(NoPolicy_2018[[as.character(Read_CD_LINKS_2018[i,3])]], region==as.character(Read_CD_LINKS_2018[i,2]), year >= 1990, year <= 2050) %>% select(year, region, value, unit))
  }
...
{ print(paste(Read_CD_LINKS_2018[i,2], "-",Read_CD_LINKS_2018[i,3]))
  ...
  Check_targets_NPi_2018 <- rbind(Check_targets_NPi_2018, tmp_NPi_2018)
}
