...
countries_eff_cars = c("BRA", "CAN","INDIA", "JAP", "KOR", "MEX", "USA","EU", "WEU", "CEU", "CHN", "OCE")
...
emint_cars <- rbind(NoPolicy_emint_cars, NPi_emint_cars)
em_cars <- rbind(NoPolicy_em_cars, NPi_em_cars)
data_scenarios <- eff_cars_existing_scenarios; variable = "fuel efficiency cars"
#eff_cars_2020_2030 <- filter(eff_cars_scenarios, year%in%c(2017, 2020, 2021, 2022, 2025, 2030), region%in%countries_eff_cars)
g<-ggplot(data=filter(data_scenarios, region%in%countries_eff_cars, scenario%in%c("NoPolicy", "NPi"))) +
