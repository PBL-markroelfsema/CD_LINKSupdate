...
g<-ggplot(data=filter(HvyT_energy_tax, year==2030, region==country_HvyT)) + geom_bar(aes(x=scenario, y=value, fill=scenario), stat="identity") +
  ...
  scale_fill_brewer(palette="Dark2") +
  ylim(0, NA)
f = paste0("graphs/fuel efficiency/impact tax from NoPolicy on fuel efficiency heavy trucks_", country_HvyT, ".jpg")
plot(g)
ggsave(f, g)
...
ggplot(data=filter(HvyT_NPi_energy_tax, year==2030)) + geom_bar(aes(x=scenario, y=value, fill=scenario), stat="identity") +
  ...
  #scale_fill_brewer(palette="Dark2") +
  ylim(0, NA)
f = paste0("graphs/fuel efficiency/impact tax from NPi on fuel efficiency heavy trucks_", country, ".jpg")
ggsave(f, g)
...
Test_HvyT_1_em <- filter(Test_HvyT_1$TransportFreightCO2Emissions, year>=2015, year<=2030, travel_mode=="Heavy truck")  %>% mutate(scenario="1.0")
Test_HvyT_2_em <- filter(Test_HvyT_2$TransportFreightCO2Emissions, year>=2015, year<=2030, travel_mode=="Heavy truck") %>% mutate(scenario="2.0")
Test_HvyT_3_em <- filter(Test_HvyT_3$TransportFreightCO2Emissions, year>=2015, year<=2030, travel_mode=="Heavy truck") %>% mutate(scenario="3.0")
Test_HvyT_5_em <- filter(Test_HvyT_5$TransportFreightCO2Emissions, year>=2015, year<=2030, travel_mode=="Heavy truck") %>% mutate(scenario="5.0")
Test_HvyT_10_em <- filter(Test_HvyT_10$TransportFreightCO2Emissions, year>=2015, year<=2030, travel_mode=="Heavy truck") %>% mutate(scenario="10")
Test_HvyT_15_em <- filter(Test_HvyT_15$TransportFreightCO2Emissions, year>=2015, year<=2030, travel_mode=="Heavy truck") %>% mutate(scenario="15")
em_HvyT <- rbind(Test_HvyT_1_em, Test_HvyT_2_em) %>% rbind(Test_HvyT_3_em) %>% rbind(Test_HvyT_5_em) %>% rbind(Test_HvyT_10_em) %>% rbind(Test_HvyT_15_em)
em_HvyT$scenario <- factor(em_HvyT$scenario, c("1.0","2.0", "3.0", "5.0", "10", "15"))
...
# CARS
TIMER_model_folder = paste("~/disks/y/Kennisbasis/IMAGE/model/users/mark/timer/CD_LINKSupdate/2_TIMER/TIMER_2015_SSPs_ClimatePolicies/ENERGY/ENDATREG/data/endem",sep="/")
...
d1 <- filter(NoPolicy_2019$EfficiencyFreight, region==country, mode=="Heavy truck") %>% mutate(scenario="NoPolicy")
...
# Determine efficiency heavy trucks (in order)
e1 <- filter(NoPolicy_2019$EfficiencyFreight, region%in%countries_HvyT, mode=="Heavy truck", year==2010) %>% select(-year)
...
#eff_HvyT <- eff_HvyT_new
eff_HvyT <- eff_HvyT_existing
g<-ggplot(data=filter(eff_HvyT, region%in%countries_HvyT, scenario%in%c("1.0","2.0", "3.0", "5.0", "10", "15"))) +
  geom_point(aes(x=year, y=value, colour=scenario, shape=standard), size=5) +
  geom_line(aes(x=year, y=value, colour=scenario, linetype=standard))+ 
  facet_wrap(~region)+
  scale_colour_brewer(palette="Dark2") +
  ggtitle(paste0("Fuel efficiency new and existing HvyT fleet (from NoPolicy) ", countries_HvyT)) +
  ylab(eff_HvyT$unit) +
  theme_bw() +
  ylim(0, NA)
f = paste0("graphs/fuel efficiency/compare - impact tax - fuel efficiency new and existing heavy trucks NoPolicy", countries_HvyT, ".jpg")
plot(g)
ggsave(f, g)
...
g<-ggplot(data=filter(em_HvyT, region%in%countries_HvyT, scenario%in%c("1.0","2.0", "3.0", "5.0", "10", "15"))) +
  geom_point(aes(x=year, y=value, colour=scenario), size=5) +
  ...
  theme_bw() +
  ylim(0, NA)
f = paste0("graphs/fuel efficiency/compare - impact tax on GHG emissions heavy trucks NoPolicy", countries_HvyT, ".jpg")
plot(g)
ggsave(f, g)
