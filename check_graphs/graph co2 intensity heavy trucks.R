...
# compare electric share heavy trucks and new heavy trucks
r = c('CHN', 'EU')
d1 <- filter(NPi$ElectricShare_HvyT, region%in%r, year>=2010, year<=2050) %>% mutate(vintage='existing')
d2 <- filter(NPi$ElectricShare_new_HvyT, region%in%r, year>=2010, year<=2050) %>% mutate(vintage='new')
