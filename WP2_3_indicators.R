...
regions <- c("BRA", "CHN", "EU", "IND", "JPN", "RUS", "USA", "World", "ROW", "Bunkers")
...
NoPolicy_ind <- ProcessTimerScenario(NoPolicy, Rundir, Project, Policy=TRUE)
NPi_ind <- ProcessTimerScenario(NPi, Rundir, Project, Policy=TRUE)
INDCi_ind <- ProcessTimerScenario(INDCi, Rundir, Project, Policy=TRUE)
INDC2030i_1000_ind <- ProcessTimerScenario(INDC2030i_1000, Rundir, Project, Policy=TRUE)
NPi2020_1600_ind <- ProcessTimerScenario(NPi2020_1600, Rundir, Project, Policy=TRUE)
NPi2020_1000_ind <- ProcessTimerScenario(NPi2020_1000, Rundir, Project, Policy=TRUE)
NPi2020_400_ind <- ProcessTimerScenario(NPi2020_400, Rundir, Project, Policy=TRUE)
...
write.table(PRIMAP_selec_Kyoto_output, file="data/History_Kyoto.csv", sep=";", row.names = FALSE)
...
PRIMAP_selec_Gases <- rbind(PRIMAP_selec_Gases, PRIMAP_selec_Kyoto)
...
RoW_Gases_hist <- group_by(Countries_Gases_hist, variable, scenario, unit, source, statistic, year) %>% summarise(value=sum(value,na.rm=TRUE))
...
write.table(PRIMAP_selec_Gases_output, file="data/History_AllGases.csv", sep=";", row.names = FALSE)

# Historical data Figure 4------------------------------------------
Kyoto_hist <- gather(PRIMAP_selec_Kyoto, 7:ncol(PRIMAP_selec_Kyoto), key="year", value=value)
Kyoto_hist <- filter(Kyoto_hist, year>=1970, year<=2015)
...
CO2_hist <- filter(CO2_hist, year>=1970, year<=2015)
...
# Use OECD GDP history from IMAGE model
...
GDP_MER_hist <- filter(GDP_MER_IMAGE, year>=1970, year<=2015, region %in% regions)
GDP_MER_hist$region <- factor(GDP_MER_hist$region, levels=regions)
GHG_intensity_hist <- inner_join(GHG_hist, GDP_MER_hist, by=c('year', 'region'))
...
GHG_intensity_hist[GHG_intensity_hist[,"variable"]=="Emissions|CO2", "variable"] <- "Carbon Intensity of GDP|MER"
GHG_intensity_hist$scenario <- "History"
GHG_intensity_hist <- mutate(GHG_intensity_hist, unit="Mt CO2eq/million US($2005)")
GHG_intensity_hist$source <- "PRIMAP, SSP database"
...
GHG_intensity_rate_hist <- group_by(GHG_intensity_hist, variable, scenario, region, unit, source) %>% mutate(prev=lag(value)) %>% 
                           mutate(rate=value/prev-1) %>%
                           select(variable, scenario, region, unit, source, statistic, year, rate) %>%
                           filter(year>=1980)
colnames(GHG_intensity_rate_hist)[colnames(GHG_intensity_rate_hist)=="rate"] <- "value"

GHG_intensity_5yr_hist <- filter(GHG_intensity_hist, year %in% c(1975, 1980, 1985, 1990, 1995, 2000, 2005, 2010, 2015))
GHG_intensity_rate_annual5yrperiod_hist <- group_by(GHG_intensity_5yr_hist, variable, scenario, region, unit, source) %>% mutate(prev=lag(value)) %>% 
...
write.table(GHG_intensity_rate_annual5yrperiod_hist, file="data/History_GHG_Intensity.csv", sep=";", row.names = FALSE)
...
write.table(d_selec_Kyoto_stat, file="data/Figure1_Kyoto.csv", sep=";", row.names = FALSE)
...
q1_2 <- ggplot(data=filter(data_fig1, statistic=="median")) +  
     geom_line(aes(year, value, colour=region, linetype=scenario,  group=interaction(scenario, region))) + 
     #scale_linetype_manual(values=seq(1,7)) + 
...
write.table(d_cd_links_GHG_countries, file="data/temp2.csv", sep=";", row.names = FALSE)
...
d_cd_links_GHG_countries_median <- rbind(d_cd_links_GHG_countries_stat, d_selec_Kyoto_median)
...
write.table(d_cd_links_GHG_countries_stat , file="data/figure2_3.csv", sep=";", row.names = FALSE)
...
#  Data Figure 4 ---------------------------------------------------------------
# Carbon intensity 
intens=filter(all_cd_links, variable%in%c("Carbon Intensity of GDP|MER","GHG Intensity of GDP|MER"),
...
write.table(intensrate_stat,"data/Figure4_Cintensityraterange.csv", sep=";", row.names = FALSE)
...
write.table(data_figure5_stat_fig , file="data/figure5_stat.csv", sep=";", row.names = FALSE)
...
NoPolicy_fig6_elec <- filter(NoPolicy_ind$NonFossilElecShare, year>=2010, year<=2050) %>% mutate(scenario="No policy") %>% mutate(sector="Electricity")
NPi_fig6_elec  <- filter(NPi_ind$NonFossilElecShare, year>=2010, year<=2050) %>% mutate(scenario="National Policies") %>% mutate(sector="Electricity")
INDCi_fig6_elec <- filter(INDCi_ind$NonFossilElecShare, year>=2010, year<=2050) %>% mutate(scenario="NDC") %>% mutate(sector="Electricity")
INDC2030i_1600_fig6_elec <- filter(INDC2030i_1000_ind$NonFossilElecShare, year>=2010, year<=2050) %>% mutate(scenario="2C_66 (2030)") %>% mutate(sector="Electricity")
NPi2020_1600_fig6_elec <- filter(NPi2020_1000_ind$NonFossilElecShare, year>=2010, year<=2050) %>% mutate(scenario="2C_50") %>% mutate(sector="Electricity")
NPi2020_1000_fig6_elec <- filter(NPi2020_1000_ind$NonFossilElecShare, year>=2010, year<=2050) %>% mutate(scenario="2C_66") %>% mutate(sector="Electricity")
NPi2020_400_fig6_elec <- filter(NPi2020_400_ind$NonFossilElecShare, year>=2010, year<=2050) %>% mutate(scenario="1.5C_50") %>% mutate(sector="Electricity")
...
NoPolicy_fig6_res <- filter(NoPolicy_ind$NonFossilResBuildingsShare, year>=2010, year<=2050, population_group=="Total") %>% mutate(scenario="No policy") %>% mutate(sector="Residential buildings")
NPi_fig6_res  <- filter(NPi_ind$NonFossilResBuildingsShare, year>=2010, year<=2050, population_group=="Total") %>% mutate(scenario="National Policies") %>% mutate(sector="Residential buildings")
INDCi_fig6_res <- filter(INDCi_ind$NonFossilResBuildingsShare, year>=2010, year<=2050, population_group=="Total") %>% mutate(scenario="NDC") %>% mutate(sector="Residential buildings")
INDC2030i_1600_fig6_res <- filter(INDC2030i_1000_ind$NonFossilResBuildingsShare, year>=2010, year<=2050, population_group=="Total") %>% mutate(scenario="2C_66 (2030)", sector="Residential buildings")
NPi2020_1600_fig6_res <- filter(NPi2020_1600_ind$NonFossilResBuildingsShare, year>=2010, year<=2050, population_group=="Total") %>% mutate(scenario="2C_50") %>% mutate(sector="Residential buildings")
NPi2020_1000_fig6_res <- filter(NPi2020_1000_ind$NonFossilResBuildingsShare, year>=2010, year<=2050, population_group=="Total") %>% mutate(scenario="2C_66") %>% mutate(sector="Residential buildings")
NPi2020_400_fig6_res <- filter(NPi2020_400_ind$NonFossilResBuildingsShare, year>=2010, year<=2050, population_group=="Total") %>% mutate(scenario="1.5C_50") %>% mutate(sector="Residential buildings")
...
NoPolicy_fig6_transport <- filter(NoPolicy_ind$NonFossilTransportShare, year>=2010, year<=2050, travel_mode=="Total", type=="Total") %>% mutate(scenario="No policy") %>% mutate(sector="Transport")
NPi_fig6_transport  <- filter(NPi_ind$NonFossilTransportShare, year>=2010, year<=2050, travel_mode=="Total", type=="Total") %>% mutate(scenario="National Policies") %>% mutate(sector="Transport")
INDCi_fig6_transport <- filter(INDCi_ind$NonFossilTransportShare, year>=2010, year<=2050, travel_mode=="Total", type=="Total") %>% mutate(scenario="NDC") %>% mutate(sector="Transport")
INDC2030i_1600_fig6_transport <- filter(INDC2030i_1000_ind$NonFossilTransportShare, year>=2010, year<=2050, travel_mode=="Total", type=="Total") %>% mutate(scenario="2C_66 (2030)") %>% mutate(sector="Transport")
NPi2020_1600_fig6_transport <- filter(NPi2020_1600_ind$NonFossilTransportShare, year>=2010, year<=2050, travel_mode=="Total", type=="Total") %>% mutate(scenario="2C_50") %>% mutate(sector="Transport")
NPi2020_1000_fig6_transport <- filter(NPi2020_1000_ind$NonFossilTransportShare, year>=2010, year<=2050, travel_mode=="Total", type=="Total") %>% mutate(scenario="2C_66") %>% mutate(sector="Transport")
NPi2020_400_fig6_transport <- filter(NPi2020_400_ind$NonFossilTransportShare, year>=2010, year<=2050, travel_mode=="Total", type=="Total") %>% mutate(scenario="1.5C_50") %>% mutate(sector="Transport")
...
write.table(data_figure6 , file="data/figure6.csv", sep=";", row.names = FALSE)
